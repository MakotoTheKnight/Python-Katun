...
CREATE TRIGGER deleteSong BEFORE DELETE ON song
BEGIN
	DELETE FROM album
...
CREATE TRIGGER checkDuplicates AFTER INSERT ON song
BEGIN
...
CREATE TRIGGER updateName AFTER UPDATE OF pname on playlist
BEGIN
	UPDATE contains SET pname = playlist.pname WHERE contains.pname = old.pname;
END;

CREATE TRIGGER deleteName AFTER DELETE ON playlist
BEGIN
